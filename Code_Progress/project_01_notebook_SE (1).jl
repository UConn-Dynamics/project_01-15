...
# ╔═╡ c5b83d8f-7d89-4235-baba-ec6c45bbd395
begin
	md"## Interactive Rotation Speed Exploration"
end

# ╔═╡ 93b1b5c9-d9bc-48bb-b1bf-780c4e3986fa
begin
	@bind Ω_slider Slider(0:0.5:20, default=2, show_value=true)
end

# ╔═╡ 6162ff3a-707e-4d94-96a8-ea4f0481a307
begin
    θ0_int = 0.2
    ω0_int = 0.0
    u0_int = [θ0_int, ω0_int]

    tspan_int = (0.0,20.0)

    function rotating_pendulum_int!(du,u,p,t)
        θ, ω = u
        g = 9.81
        L = 1.0

        du[1] = ω
        du[2] = -(g/L)*sin(θ) + Ω_slider^2*sin(θ)*cos(θ)
    end

    prob_int = ODEProblem(rotating_pendulum_int!,u0_int,tspan_int)
    sol_int = solve(prob_int)

    plot(sol_int.t, sol_int[1,:],
        xlabel="Time (s)",
        ylabel="Angle θ (rad)",
        title="Pendulum Motion with Rotation Speed Ω = $Ω_slider",
        linewidth=3,
        legend=false)
end
...
# ╔═╡ 3bc5b62b-18e7-4e31-91ff-432dee9b2095
begin

    g_num = 9.81
    L_num = 0.15   
    Ωcrit = sqrt(g_num / L_num)

    tilted_equilibrium_angles(Ω::Real; g=g_num, L=L_num) = begin
        Ω = Float64(Ω)
        c = g / (Ω^2 * L)
        if c > 1.0
            Float64[]
    ...
    stability_flags(Ω::Real; g=g_num, L=L_num) = begin
        Ω = Float64(Ω)
        Ωcrit_local = sqrt(g / L)
        stable0 = Ω < Ωcrit_local
        stablepi = false
        stabletilt = Ω >= Ωcrit_local
        stable0, stablepi, stabletilt
    end
end

# ╔═╡ 7f5d662f-65fa-497e-8046-ebd852f9a542
begin
    # Compute equilibrium/stability data 
    Ω_min = 0.0
    Ω_max = 12.0
    NΩ = 300
    Ωs = Float64.(collect(range(Ω_min, Ω_max, length=NΩ))) 

    θ0 = zeros(Float64, NΩ)
    θπ = fill(pi, NΩ)

    θtilt_pos = fill(NaN, NΩ)
    θtilt_neg = fill(NaN, NΩ)

    stable0 = falses(NΩ)
    stableπ = falses(NΩ)
    stabletilt = falses(NΩ)

    for (i, Ω) in enumerate(Ωs)
        a = tilted_equilibrium_angles(Ω)
        if length(a) == 2
            θtilt_pos[i] = a[1]
            θtilt_neg[i] = a[2]
        end

        s0, sπ, st = stability_flags(Ω)
        stable0[i] = s0
        stableπ[i] = sπ
        stabletilt[i] = st
    end

    md"Equilibrium & stability data computed.  Ωcrit = $(round(Ωcrit, digits=3)) rad/s"
end

# ╔═╡ 5ea6facc-c8dc-40cf-9c9d-a7d16ba6b168
begin
    θ0_deg = θ0 .* (180/pi)
    θtilt_pos_deg = θtilt_pos .* (180/pi)
    θtilt_neg_deg = θtilt_neg .* (180/pi)

    p = plot(
        xlabel="Ω (rad/s)",
        ylabel="Equilibrium angle θ* (deg)",
        title="Stable Equilibria vs Rotation Speed",
        legend=:topright
    )

    plot!(p, Ωs[stable0], θ0_deg[stable0], label="θ*=0 (stable)", linewidth=3)
    plot!(p, Ωs[.!stable0], θ0_deg[.!stable0], label="θ*=0 (unstable)", linestyle=:dash, linewidth=3)

    idx = .!isnan.(θtilt_pos)
    plot!(p, Ωs[idx], θtilt_pos_deg[idx], label="tilted θ* (stable)", linewidth=3)
    plot!(p, Ωs[idx], θtilt_neg_deg[idx], label="", linewidth=3)

    vline!(p, [Ωcrit], label="Ωcrit", linestyle=:dot, linewidth=2)
...
5. **Stablity Mapping** The dashed line at ΩCrit = 7.8rad/s is the threshold where the downward equilibrium (θ=0) changes stability. For Ω < ΩCrit the pendulum is stable and oscillates straight down. When Ω> ΩCrit you lose stability of the straight down positon. The stable equlibria is shifted to the titled angles shown by the two curved brances. 
...
manifest_format = "2.0"
project_hash = "2494dc1118308fc5240a37124d0095ee9959cc99"
